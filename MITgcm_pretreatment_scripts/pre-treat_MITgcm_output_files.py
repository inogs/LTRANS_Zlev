import os,sys
LTRANS_tools="/g100_work/OGS23_PRACE_IT/LAGRANGIAN/LTRANS_tools/"
PACKAGEDIRECTORY="/g100_work/OGS23_PRACE_IT/LAGRANGIAN/NECTON/5yrs_new_GGL/LTRANS_Zlev/"
Gridname='5yrs_new_GGL'
MITgcmdirectory="/g100_scratch/userexternal/fgiordan/MITgcm_experiments/"+Gridname+"/run/"
#
writeUniformIniParlocFile=True
Reconstruct_missing_coordinates=True
...
if(True):
        ...
        filename=PACKAGEDIRECTORY+'plot_scripts/setup_'+identifier+'.py'
...
for nf in range(0,numgridfiles):
       ...
       #try:
       #  dataread= struct.unpack('d'*full_nk*full_nyrho_in*full_nxrho_in ,f.read(8*full_nk*full_nyrho_in*full_nxrho_in))
       #except:
       #  dataread= struct.unpack('f'*full_nk*full_nyrho_in*full_nxrho_in,f.read(4*full_nk*full_nyrho_in*full_nxrho_in))
        ...
        if(nf==nfXC) : 
                ...
                if(Reconstruct_missing_coordinates):
                  jlist,ilist=np.where(lon_rho<=0)
                  for j,i in zip(jlist,ilist):
                    jgood=np.where(lon_rho[:,i]>0)[0][0]
                    lon_rho[j,i]=lon_rho[jgood,i] 
                ...
                if(Reconstruct_missing_coordinates):
                  jlist,ilist=np.where(lat_rho<=0)
                  for j,i in zip(jlist,ilist):
                    igood=np.where(lat_rho[j,:]>0)[0][0]
                    lat_rho[j,i]=lat_rho[j,igood] 
