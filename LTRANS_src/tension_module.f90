...
! http://www.acm.org/publications/policies/softwarecrnotice. 
  ...
  PUBLIC
  SAVE
  INTEGER, ALLOCATABLE, DIMENSION(:) :: OMP_IL
CONTAINS
...
!   and second differences D1 and D2.
      if(IP1>N .or. I>N)then
           write(*,*)'tension spline module ',N,I,IP1,T,X
      endif 
      DX = X(IP1) - X(I)
      IF (DX .LE. 0.D0) THEN
        ! X(I) .GE. X(I+1).
...
!***********************************************************
  ...
  INTEGER FUNCTION INTRVL (T,N,XINTERVL)
    !$ use OMP_LIB
    INTEGER N
    DOUBLE PRECISION T, XINTERVL(N)
!***********************************************************
!                                                From TSPACK
!                                            Robert J. Renka
!                                  Dept. of Computer Science
...
! XINTERVL(IL) .LE. T .LT. XINTERVL(IL+1)  or  (T .LT. XINTERVL(1) and IL=1)
!                            or  (T .GE. XINTERVL(N) and IL=N-1)
